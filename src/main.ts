...
//  RectangleTool
const rectangleTool = new RectangleTool(ctx);
...
class Stroke {
  ...
  updatePosition(x: number, y: number) {
    if (this.points.length < 1) this.position = { x: x, y: y };
  }
...
function redraw() {
    ...
    // Draw rectangles from RectangleTool
    rectangleTool.drawAllRectangles();
...
clearButton.addEventListener("click", () => {
  if (ctx) {
    ...
    rectangleTool.resetRectangleCount();
  }
});
